...
test.describe("emptystate", () => {
  ...
  test("emptystate / compact / dark — L1 axe: color-contrast (WCAG 1.4.3)", async ({ page }) => {
  ...
  Element has insufficient color contrast of 3.88 (foreground color: #5696ff, background color: #303a53, font size: 10.5pt (14px), font weight: normal). Expected contrast ratio of 4.5:1
    ...
    expect(violations,
      `axe rule "color-contrast" must not fire on emptystate/compact/dark`)
      .toHaveLength(0);
  });

  test("emptystate / compact / dark — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
  ...
  var el = preview.querySelector("[class*=\"jera-emptystate\"]")
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — emptystate / compact / dark`)
