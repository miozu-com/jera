...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/button/`;

test.describe("button", () => {

  test("button / success / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 3.77:1  Required: 4.5:1  FG: rgb(0,138,62)  BG: rgb(225,239,233)  Font: 16px
  ...
  var el = preview.querySelector("[class*=\"jera-button\"]")
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — button / success / light`)
      .toBeGreaterThanOrEqual(result!.required);
  });

  test("button / danger / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 4.44:1  Required: 4.5:1  FG: rgb(204,39,46)  BG: rgb(246,229,232)  Font: 16px
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result, 'Could not locate jera element for contrast measurement').not.toBeNull();
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — button / danger / light`)
      .toBeGreaterThanOrEqual(result!.required);
  });

  test("button / warning / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 3.18:1  Required: 4.5:1  FG: rgb(172,125,0)  BG: rgb(242,238,227)  Font: 16px
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result, 'Could not locate jera element for contrast measurement').not.toBeNull();
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — button / warning / light`)
      .toBeGreaterThanOrEqual(result!.required);
  });

  test("button / info / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 3.98:1  Required: 4.5:1  FG: rgb(0,127,146)  BG: rgb(225,238,242)  Font: 16px
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result, 'Could not locate jera element for contrast measurement').not.toBeNull();
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — button / info / light`)
      .toBeGreaterThanOrEqual(result!.required);
  });

  test("button / primary / dark — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 4.39:1  Required: 4.5:1  FG: rgb(86,150,255)  BG: rgb(40,50,72)  Font: 16px
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result, 'Could not locate jera element for contrast measurement').not.toBeNull();
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — button / primary / dark`)
      .toBeGreaterThanOrEqual(result!.required);
  });
});
