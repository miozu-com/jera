...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/pin-input/`;

test.describe("pin-input", () => {

  test("pin-input / sm / light — L1 axe: label (WCAG 4.1.2)", async ({ page }) => {
  ...
  Elem
    ...
    expect(violations,
      `axe rule "label" must not fire on pin-input/sm/light`)
      .toHaveLength(0);
  });

  test("pin-input / sm / L2 keyboard: enter-key-no-activate (WCAG 2.1.1)", async ({ page }) => {
  ...
  test("pin-input / md / L2 keyboard: enter-key-no-activate (WCAG 2.1.1)", async ({ page }) => {
    // L2 Interaction — enter-key-no-activate — WCAG 2.1.1
    // Fix: Enter key does not activate component — add keydown handler or use native button element
    await page.goto(COMPONENT_URL);
    await page.waitForSelector('.preview-area', { timeout: 15000 });
  ...
  test("pin-input / lg / L2 keyboard: enter-key-no-activate (WCAG 2.1.1)", async ({ page }) => {
    // L2 Interaction — enter-key-no-activate — WCAG 2.1.1
    // Fix: Enter key does not activate component — add keydown handler or use native button element
    await page.goto(COMPONENT_URL);
    await page.waitForSelector('.preview-area', { timeout: 15000 });
