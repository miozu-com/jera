...
test.describe("code-block", () => {
  ...
  test("code-block / javascript / light — L1 axe: color-contrast (WCAG 1.4.3)", async ({ page }) => {
  ...
  Element has insufficient color contrast of 4.31 (foreground color: #008a3e, background color: #fafbfd, font size: 10.5pt (14px), font weight: normal). Expected contrast ratio of 4.5:1
    ...
    expect(violations,
      `axe rule "color-contrast" must not fire on code-block/javascript/light`)
      .toHaveLength(0);
  });
  ...
  test("code-block / L4 mobile (375×812) — text_readable", async ({ page }) => {
  ...
  test("code-block / L4 tablet (768×1024) — overflow", async ({ page }) => {
  ...
  test("code-block / L4 tablet (768×1024) — text_readable", async ({ page }) => {
  ...
  test("code-block / L4 desktop (1440×900) — text_readable", async ({ page }) => {
