...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/select/`;

test.describe("select", () => {

  test("select / xs / light — L1 axe: select-name (WCAG 4.1.2)", async ({ page }) => {
    // axe violation: select-name — impact: critical — WCAG 4.1.2
  ...
  Elem
    ...
    await page.selectOption('.controls-panel select.jera-select', "xs").catch(() => {});
    ...
    const results = await new AxeBuilder({ page })
      ...
      .analyze();
    const violations = results.violations.filter(v => v.id === "select-name");
    expect(violations,
      `axe rule "select-name" must not fire on select/xs/light`)
      .toHaveLength(0);
  });
});
