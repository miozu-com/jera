...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/toast/`;

test.describe("toast", () => {

  test("toast / info / dark — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
  ...
  var el = preview.querySelector("[class*=\"jera-toast\"]")
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — toast / info / dark`)
