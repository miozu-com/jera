...
test.describe("radiogroup", () => {
  ...
  test("radiogroup / default / light — L1 axe: color-contrast (WCAG 1.4.3)", async ({ page }) => {
  ...
  Element has insufficient color contrast of 4.29 (foreground color: #007f92, background color: #f3f4f7, font size: 9.0pt (12px), font weight: normal). Expected contrast ratio of 4.5:1
    ...
    expect(violations,
      `axe rule "color-contrast" must not fire on radiogroup/default/light`)
      .toHaveLength(0);
  });

  test("radiogroup / default / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
  ...
  var el = preview.querySelector("[class*=\"jera-radiogroup\"]")
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — radiogroup / default / light`)
