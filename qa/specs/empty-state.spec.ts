...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/empty-state/`;

test.describe("empty-state", () => {

  test("empty-state / compact / dark — L1 axe: color-contrast (WCAG 1.4.3)", async ({ page }) => {
    // axe violation: color-contrast — impact: serious — WCAG 1.4.3
    // Fix: Fix any of the following:
  Element has insufficient color contrast of 3.88 (foreground color: #5696ff, background color: #303a53, font size: 10.5pt (14px), font weight: normal). Expected contrast ratio of 4.5:1
    ...
    expect(violations,
      `axe rule "color-contrast" must not fire on empty-state/compact/dark`)
      .toHaveLength(0);
  });

  test("empty-state / compact / dark — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 4.39:1  Required: 4.5:1  FG: rgb(86,150,255)  BG: rgb(40,50,72)  Font: 14px
  ...
  var el = preview.querySelector("[class*=\"jera-empty-state\"]")
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — empty-state / compact / dark`)
