...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/radio/`;

test.describe("radio", () => {

  test("radio / vertical / L2 keyboard: enter-key-no-activate (WCAG 2.1.1)", async ({ page }) => {
    ...
    await page.selectOption('.controls-panel select.jera-select', "vertical").catch(() => {});
  ...
  test("radio / horizontal / L2 keyboard: enter-key-no-activate (WCAG 2.1.1)", async ({ page }) => {
    // L2 Interaction — enter-key-no-activate — WCAG 2.1.1
    // Fix: Enter key does not activate component — add keydown handler or use native button element
    await page.goto(COMPONENT_URL);
    await page.waitForSelector('.preview-area', { timeout: 15000 });
    await page.waitForTimeout(500);
    await page.selectOption('.controls-panel select.jera-select', "horizontal").catch(() => {});
    await page.waitForTimeout(300);
