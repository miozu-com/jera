...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/tooltip/`;

test.describe("tooltip", () => {

  test("tooltip / top / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 4.12:1  Required: 4.5:1  FG: rgb(16,95,217)  BG: rgb(203,220,245)  Font: 16px
    // Fix: Contrast ratio 4.12:1 below WCAG AA minimum of 4.5:1
    await page.goto(COMPONENT_URL);
    await page.waitForSelector('.preview-area', { timeout: 15000 });
    await page.waitForTimeout(500);
    await page.selectOption('.controls-panel select.jera-select', "top").catch(() => {});
  ...
  var el = preview.querySelector("[class*=\"jera-tooltip\"]")
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — tooltip / top / light`)
