...
const COMPONENT_URL = `${STORYBOOK_URL}/docs/components/alert/`;

test.describe("alert", () => {

  test("alert / success / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 3.89:1  Required: 4.5:1  FG: rgb(0,138,62)  BG: rgb(230,242,238)  Font: 16px
    // Fix: Contrast ratio 3.89:1 below WCAG AA minimum of 4.5:1
  ...
  var el = preview.querySelector("[class*=\"jera-alert\"]")
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — alert / success / light`)
      .toBeGreaterThanOrEqual(result!.required);
  });

  test("alert / warning / light — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 3.27:1  Required: 4.5:1  FG: rgb(172,125,0)  BG: rgb(244,241,233)  Font: 16px
    // Fix: Contrast ratio 3.27:1 below WCAG AA minimum of 4.5:1
    await page.goto(COMPONENT_URL);
    await page.waitForSelector('.preview-area', { timeout: 15000 });
    await page.waitForTimeout(500);
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result, 'Could not locate jera element for contrast measurement').not.toBeNull();
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — alert / warning / light`)
      .toBeGreaterThanOrEqual(result!.required);
  });

  test("alert / error / dark — L1 contrast ratio must meet WCAG AA (1.4.3)", async ({ page }) => {
    // Canvas-sampled contrast — axe marks oklch tokens as 'incomplete', we detect via pixel sampling
    // Measured: 4.32:1  Required: 4.5:1  FG: rgb(252,88,85)  BG: rgb(52,43,54)  Font: 16px
    // Fix: Contrast ratio 4.32:1 below WCAG AA minimum of 4.5:1
    await page.goto(COMPONENT_URL);
    await page.waitForSelector('.preview-area', { timeout: 15000 });
    await page.waitForTimeout(500);
  ...
  return { ratio: ratio, required: required, fg: "rgb("+fg+")", bg: "rgb("+bg+")" };
    ...
    expect(result, 'Could not locate jera element for contrast measurement').not.toBeNull();
    expect(result!.ratio,
      `Contrast ${result!.ratio}:1 must meet WCAG AA (${result!.required}:1) — alert / error / dark`)
      .toBeGreaterThanOrEqual(result!.required);
  });
});
